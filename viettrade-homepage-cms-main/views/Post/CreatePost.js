...
const CreatePost = () => {
  ...
  const categoryIds = dataCategory?.map((category) => {
    ...
    <div className="card radius-15">
      <div className="card-body">
        <div className="card-title">
          <h2 className="text-[20px] font-semibold">Thêm mới tin tức</h2>
          <div className="mt-4">
            <FormCreate dataCategory={categoryIds} />
          </div>
        </div>
      </div>
    </div>
...
const FormCreate = ({ dataCategory }) => {
  const { t } = useTranslation("common");
  const [tab, setTab] = useState(1);
  const { mutate, data: dataCreate, isSuccess, isLoading } = useCreatePost();
  ...
  const [categoryIdSelected, setCategoryId] = useState("");
  const [publishSelected, setPublish] = useState("");
  ...
  const schema = yup.object().shape({
    ...
    description_vi: yup.string().trim().required("Mô tả là bắt buộc"),
    title_en: yup
      .string()
      .trim()
    ...
    description_en: yup.string().trim().required("Mô tả tiếng anh là bắt buộc"),
    feature_image: yup
      .mixed()
    ...
    publishSelected: yup
      .object()
      .required("Trường công khai không được bỏ trống"),
    categorySelected: yup
      .object()
      .required("Trường thể loại không được bỏ trống"),
  });
  ...
  const handleDocumentChange = (e) => {
    if (e.target.files.length) {
      setDocument(Object.values(e.target.files));
    }
  };
  ...
  const handleChangePublishStatus = (e) => {
    setPublish(e.value);
  };
  return (
            ...
            children: (
              <>
                ...
                <div className="mt-3">
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, value, ref } }) => (
                      <Select
                        ...
                        value={value}
                        className={`${errors.publishSelected && "is-invalid"}`}
                        onChange={(val) => {
                          onChange(val);
                  ...
                  {errors.publishSelected && (
                    <p className="text-danger">
                      {errors.publishSelected.message}
                    </p>
                  )}
                </div>
              </>
          ...
          <div className="form-group mc-3 ">
            ...
            <div className="input-group mb-3">
              <input
                ...
                className="form-control"
                id="inputGroupFile01"
                {...register("feature_document", { required: true })}
