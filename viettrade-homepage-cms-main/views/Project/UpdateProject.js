...
import { useDetailProject, useUpdateProject } from "../../hooks/project";
import noImage from "../../public/images/no-image.png";
import { ellipsShortNameFile } from "~/utils/string";

const UpdateProject = ({ projectId }) => {
  const { t } = useTranslation("common");
  const { data: { data: project } = {}, isLoading } =
    useDetailProject(projectId);

  return (
    <div className="card radius-15">
      <div className="card-body">
        <div className="card-title">
          <h4 className="text-[18px] font-semibold">Cập nhật dự án</h4>
          <div className="mt-4">
            <FormUpdate project={project} t={t} />
            {isLoading ? (
              <div
...
const FormUpdate = ({ project, t }) => {
  const [tab, setTab] = useState(1);
  const [image, setImage] = useState({ preview: "" });
  const [document, setDocument] = useState([]);
  const [documentPre, setDocumentPre] = useState([]);
  const { mutate, data: dataUpdate, isLoading, isSuccess } = useUpdateProject();
  ...
  const schema = yup.object().shape({
    title_vi: yup
      ...
      .max(191, "Trường tên bài viết không dài quá 191 kí tự"),    
    content_vi: yup.string().trim().required("Mô tả là bắt buộc"),
    title_en: yup
      .string()
      .trim()
      .required("Tên tiếng anh dự án là bắt buộc")
      .max(191, "Trường tên dự án không dài quá 191 kí tự"),
  ...
  useEffect(() => {
    if (project?.feature_image) {
      setImage({
        preview: project?.feature_image,
      });
    }
    if (project?.feature_document) {
      setDocument(JSON.parse(project.feature_document));
    }
  }, [project]);

  const handleDocumentChange = (e) => {
    if (e.target.files.length) {
      setDocumentPre(Object.values(e.target.files[e.target.files.length - 1]));
      setDocument([]);
    } else {
      setDocument(JSON.parse(project.feature_document));
      setDocumentPre([]);
              ...
              <>
                ...
                <div className="mt-3">
                  ...
                  <Editor
                    ...
                    initialValue={project?.content_vi}
                    textareaName={project?.content_vi}
                    onChange={() =>
                    ...
                    init={{
                      ...
                      // images_upload_url: `${process.env.NEXT_PUBLIC_HOST}/project/upload`,
                      images_upload_handler: (blobInfo, progress) =>
                        new Promise((resolve, reject) => {
                          ...
                          axios
                            .post(
                              `${process.env.NEXT_PUBLIC_HOST}/project/upload`,
                              formData
                            )
                            .then((res) => {
                              const location =
              ...
              <>
                ...
                <div className="mt-3">
                  ...
                  <Editor
                    ...
                    initialValue={project?.content_en}
                    textareaName={project?.content_en}
                    onChange={() =>
      ...
      <div className="row mt-3">
        <div className="col-md-6">
          <div className="form-group mc-3">
            <label className="form-label">{t("project.feature_image")} </label>
        ...
        <div className="col-6">
          <div className="wp-preview-image">
            <img
              ...
              alt=""
              style={{height:'231px', width:'231px'}}
            />
          </div>
        </div>
      </div>
